...
app.get("/todos", async (request, response) => {
    ...
    switch(true){
        case hasPriorityAndStatusProperties(request.query):
            if(priority==="HIGH" || priority=== "MEDIUM" || priority === "LOW"){
                if (
                    ...
                    getTodosQuery = `SELECT * FORM todo WHERE status='${status}' AND priority = '${priority}';`;
        ...
        case hasCategoryAndStatus(request.query):
            if(
               ...
               if(
                   ...
                   getTodosQuery = `SELECT * FORM todo WHERE category='${category}' AND status='${status}';`;
                   data = await database.all(getTodosQuery);
                   response.send(data.map((eachItem) => outPutResult(eachItem)));
               }else{
                   response.status(400);
        ...
        case hasCategoryAndPriority(request.query):
            if(
                ...
                if(
                    ...
                    getTodosQuery = `SELECT * FORM todo WHERE category='${category}' AND priority='${priority}';`;
                    data = await database.all(getTodosQuery);
                    response.send(data.map((eachItem) => outPutResult(eachItem)));
                }else{
                    response.status(400);
        ...
        case hasStatusProperty(request.query):
            if(status === "TO DO" || status === "IN PROGRESS" || status === "DONE"){
                 getTodosQuery = `SELECT * FORM todo WHERE status = '${status}';`;
                 data = await database.all(getTodosQuery);
                 response.send(data.map((eachItem) => outPutResult(eachItem)));
            }else{
                 response.status(400);
    ...
    case hasCategoryPriority(request.query):
        if(
            ...
            getTodosQuery = `SELECT * FORM todo WHERE category='${category}';`;
            data = await database.all(getTodosQuery);
            response.send(data.map((eachItem) => outPutResult(eachItem)));
        }else{
           response.status(400);
